...
class DisNet(nn.Module):
    def __init__(self):
        ...
        self.conv1 = Conv2D(3, 64, 5, 2, 'same', self.act, norm_layer2d, use_fixup_init=True)
        self.conv2 = Conv2D(64, 128, 5, 2, 'same', self.act, norm_layer2d, use_fixup_init=True)
...
if __name__ == '__main__':
    ...
    try:
        ...
        dnet.load_state_dict(torch.load("dnet.pt"))
        gnet_optimizer.load_state_dict(torch.load("gnet_optim.pt"))
        dnet_optimizer.load_state_dict(torch.load("dnet_optim.pt"))
    ...
    for i in range(start_it, iter_count):
        ...
        if i > 0 and i % iters_per_sample == 0:
            ...
            torch.save(dnet.state_dict(), 'dnet.pt')
            torch.save(gnet_optimizer.state_dict(), 'gnet_optim.pt')
            torch.save(dnet_optimizer.state_dict(), 'dnet_optim.pt')
